...
} from "./backtestStore.ts";
import { isRiskProfile } from "./dynamicRisk.ts";
import {
  ensureHistoricalNewsCoverage,
  getHistoricalNewsGate,
  getHistoricalNewsGateForRange,
} from "./historicalNewsStore.ts";
import {
...
export const executeBacktestRun = async (
  id: string,
  ...
  try {
    ...
    const backfillPages = Math.max(
      0,
      Math.floor(config.backfillPages ?? (config.pairs.length === 1 ? 50 : 10)),
    );
    for (let index = 0; index < config.pairs.length; index += 1) {
      ...
      updateBacktestRun(id, {
        progressPair: pair,
        progressDone: index,
        progressStage: "loading history",
        progressPercent: (index / config.pairs.length) * 100,
        heartbeatAt: new Date().toISOString(),
      });
      addBacktestEvent(
        id,
        "pair_loading",
        `LOADING HISTORY · collecting ${profile.trend}/${profile.zone}/${profile.confirmation} signals and ${profile.execution} execution candles for ${pair}.`,
        pair,
      );
      const [trendCandles, zoneCandles, confirmationCandles, outcomeCandles] =
        await Promise.all([
          fetchCandleHistory(pair, profile.trend, {
      ...
      if (confirmationCandles.length) {
        addBacktestEvent(
          id,
          "news_loading",
          `LOADING NEWS · verifying stored high-impact calendar coverage for ${pair}.`,
          pair,
        );
        const firstTime = Math.floor(
          new Date(confirmationCandles[0].time).getTime() / 1000,
        );
        const lastTime = Math.floor(
          new Date(
            confirmationCandles[confirmationCandles.length - 1].time,
          ).getTime() / 1000,
        );
        const coverage = await ensureHistoricalNewsCoverage(
          firstTime,
          lastTime,
          (completed, total, weekStart) => {
            const newsPercent = total
              ? Math.round((completed / total) * 100)
              : 100;
            updateBacktestRun(id, {
              progressStage: `historical news ${newsPercent}%`,
              heartbeatAt: new Date().toISOString(),
            });
            if (completed === total || completed % 10 === 0)
              addBacktestEvent(
                id,
                "news_progress",
                `NEWS HISTORY · ${completed}/${total} missing calendar weeks fetched · latest ${weekStart}.`,
                pair,
              );
          },
        );
        addBacktestEvent(
          id,
          "news_ready",
          `NEWS READY · historical high-impact windows are stored and available for ${pair}.`,
          pair,
          coverage,
        );
      }
      addBacktestEvent(
        id,
        "pair_scanning",
        `SCANNING ${pair} · ${profile.trend} ${trendCandles.length.toLocaleString()} · ${profile.zone} ${zoneCandles.length.toLocaleString()} · ${profile.confirmation} ${confirmationCandles.length.toLocaleString()} signals · ${profile.execution} ${outcomeCandles.length.toLocaleString()} execution candles.`,
        pair,
      );
      ...
      const trades = simulateGoldilocksPair({
        ...
        confirmationCandles,
        outcomeCandles,
        historicalNewsGate: (pair, time, startTime) =>
          startTime === undefined
            ? getHistoricalNewsGate(pair, time)
            : getHistoricalNewsGateForRange(pair, startTime, time),
        onNewsRejected: () => {
          newsRejected += 1;
        },
        onProximityRejected: () => {
        ...
        onMarketHoursRejected: () => {
          marketHoursRejected += 1;
        },
        onHolidayRejected: () => {
          holidayRejected += 1;
        },
        ...
        onDepartureQualityRejected: () => {
          departureQualityRejected += 1;
        },
        onExecutionCoverageRejected: () => {
          executionCoverageRejected += 1;
        },
        onProgress: (progress) => {
          const now = Date.now();
          ...
          if (
            ...
            return;
          const overall =
            ((index + progress.percent / 100) / config.pairs.length) * 100;
          updateBacktestRun(id, {
            progressStage: progress.stage,
            progressPercent: Number(overall.toFixed(1)),
          ...
          if (bucket !== lastProgressBucket) {
            addBacktestEvent(
              id,
              "pair_progress",
              `${pair} · ${progress.stage} · ${progress.percent}%`,
              pair,
      ...
      if (newsRejected)
        addBacktestEvent(
          id,
          "news_rejected",
          `NEWS BLOCKER · rejected ${newsRejected} confirmed ${pair} setup(s) inside high-impact ±1 hour windows.`,
          pair,
          { rejectedSetups: newsRejected },
        );
      if (proximityRejected)
        addBacktestEvent(
          id,
      ...
      if (marketHoursRejected)
        addBacktestEvent(
          id,
          "weekly_market_hours_rejected",
          `WEEKLY MARKET HOURS · rejected ${marketHoursRejected} ${pair} setup(s) between Friday 16:00 and Sunday 18:00 America/New_York.`,
          pair,
          { rejectedSetups: marketHoursRejected, timeZone: "America/New_York" },
        );
      if (holidayRejected)
        addBacktestEvent(
          id,
          "historical_holiday_rejected",
          `HISTORICAL HOLIDAY - rejected ${holidayRejected} ${pair} setup(s) on configured U.S. no-trade holidays evaluated in America/New_York.`,
          pair,
          { rejectedSetups: holidayRejected, timeZone: "America/New_York" },
        );
      if (sessionRejected)
        addBacktestEvent(
          id,
          "historical_session_rejected",
          `HISTORICAL SESSION - rejected ${sessionRejected} ${pair} setup(s) because neither currency was inside its DST-aware local session.`,
          pair,
          { rejectedSetups: sessionRejected },
        );
      if (departureQualityRejected)
        addBacktestEvent(
          id,
          "departure_quality_rejected",
          `DEPARTURE QUALITY - rejected ${departureQualityRejected} ${pair} setup(s) formed by an M15 shock/rejection candle.`,
          pair,
          { rejectedSetups: departureQualityRejected },
        );
      if (executionCoverageRejected)
        addBacktestEvent(
          id,
          "execution_coverage_rejected",
          `EXECUTION COVERAGE - rejected ${executionCoverageRejected} ${pair} setup(s) because M1 data did not begin at entry time.`,
          pair,
          {
            rejectedSetups: executionCoverageRejected,
            timeframe: "M1",
            maximumEntryDelaySeconds: 60,
          },
        );
      all.push(...trades);
      ...
      updateBacktestRun(id, {
        ...
        progressStage: "pair complete",
        progressPercent: ((index + 1) / config.pairs.length) * 100,
        heartbeatAt: new Date().toISOString(),
      ...
      addBacktestEvent(
        id,
        "pair_complete",
        `PAIR COMPLETE · ${pair} · ${trades.length} trades · ${trades.filter((trade) => trade.outcome === "WIN").length} protected wins.`,
        pair,
    ...
    updateBacktestRun(id, {
      ...
      progressPair: null,
      progressDone: config.pairs.length,
    ...
    addBacktestEvent(
      id,
      "run_complete",
      `BACKTEST COMPLETE · ${all.length} trades · ${wins} wins · ${(all.length ? (wins / all.length) * 100 : 0).toFixed(1)}% win rate.`,
    );
  } catch (error) {
...
export const normalizeBacktestConfig = (
  input: Partial<BacktestRunConfig>,
): BacktestRunConfig => {
  const pairs = (input.pairs ?? forexPairs).filter((pair) =>
    forexPairs.includes(pair),
  );
  if (!pairs.length) throw new Error("Select at least one supported pair.");
  const requestedLabel = String(input.label ?? "").trim();
  const timeframeProfile = isGoldilocksTimeframeProfileId(
  ...
  const config: BacktestRunConfig = {
    ...
    label: (
      requestedLabel || getGoldilocksBacktestRunLabel(timeframeProfile)
    ).slice(0, 80),
    strategyVersion: profile.strategyVersion,
    timeframeProfile,
    backfillPages: Math.min(
      500,
      Math.max(
        0,
        Math.floor(
          input.backfillPages ??
            (timeframeProfile === "higherTimeframe" ? 25 : 10),
        ),
      ),
    ),
    startingBalance: Math.min(
      100_000_000,
      Math.max(1, Number(input.startingBalance ?? 1000)),
    ),
    leverage: [10, 20, 30, 50].includes(Number(input.leverage))
...
export const startBacktest = (input: Partial<BacktestRunConfig>) => {
  ...
  const child = spawn(
    process.execPath,
    ["--import", "tsx", "workers/backtestWorker.ts", id, encoded],
    {
      cwd: process.cwd(),
      detached: true,
      stdio: "ignore",
...
export const cancelBacktest = (id: string) => {
  ...
  if (run.status !== "queued" && run.status !== "running")
    throw new Error(`Backtest is already ${run.status}.`);
  ...
  addBacktestEvent(
    id,
    "run_cancelled",
    "BACKTEST CANCELLED · stopped by user.",
  );
