...
export default function ResearchStatus(){
  ...
  return <Page>
    ...
    <Hero>
      ...
      <Sub>Continuous campaign research on one immutable comparison window: {GOLDILOCKS_COMPARISON_WINDOW_LABEL}. Every configuration sees the same candles; the strongest eligible records remain advisory until explicitly approved.</Sub>
      <StatusRow>
    ...
    <ActivePanel>
      <ActiveIntro><h2>Current campaign run</h2><Small style={{marginTop:0}}>{currentTrial?`Campaign run ${currentTrial.id.slice(0,8)} is the configuration being measured now.`:campaign?.status==='preparing'?'The fixed 2025 candle archive is being sealed.':'The worker is waiting to claim the next configuration.'}</Small></ActiveIntro>
    ...
    <Section>
      <SectionHead><div><h2>All-time top 3 campaign runs</h2><p>Only fixed-2025 campaign runs with at least 100 trades qualify. Net R leads; when two results are within 3R, a configuration with more than 5% lower max drawdown ranks higher.</p></div><div style={{color:'#718093',fontSize:11}}>Showing {(data?.allTimeRecords??[]).length} record{(data?.allTimeRecords??[]).length===1?'':'s'}</div></SectionHead>
