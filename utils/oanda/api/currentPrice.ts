...
export const currentPrice = async (symbol: string): Promise<{ bid: string; ask: string }> => {
  ...
  if (!accountId || !token || !hostname) {
    ...
    throw new Error("Missing account ID or token");
  }

  // Adjust the hostname for streaming data
  ...
  try {
    ...
    const reader = response.body?.getReader(); // Use optional chaining for safety
    if (!reader) {
    ...
    if (error instanceof Error) {
      logToFileAsync(`Error fetching price: ${error.message}`);
      throw error; // Re-throw the error if needed
    } else {
      logToFileAsync(`Unknown error occurred`);
