...
import { marketWatchMT, MarketWatchResponseMT, ErrorMTResponse } from "./match-trader/api/market-watch.js";
import { moveTPSLMT } from "./match-trader/api/move-TPSL.js";
import { openedPositionsMT, Position } from "./match-trader/api/opened-positions.js";
...
export class TradeManager {
  ...
  public start(tradeId: string, slPrice: number, tpPrice: number, orderSide: 'BUY' | 'SELL', openPrice: number, pair: string) {
    if (this.tradeIntervals.has(tradeId)) return;
  ...
  private startContinueTrailing(tradeId: string, price: number) {
    const trade = this.trades.get(tradeId);
    if (!trade) return;
  ...
  private async checkIfPositionExists(tradeId: string): Promise<boolean> {
    ...
    return response.positions.some((p: Position) => p.id === tradeId);
  }
}
