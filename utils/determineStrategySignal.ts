...
import { determineSwingPoints, Candle as SwingCandle } from '../utils/swingLabeler.js';
import { getPipIncrement } from '../utils/shared.js';
...
export const determineStrategySignal = async (pair: string): Promise<TradeSignal | null> => {
  try {
    ...
    const swings4H = determineSwingPoints(candles4H as SwingCandle[]);
    ...
    if (pattern4H !== sameDirection) return null;
    ...
    const swings1H = determineSwingPoints(candles1H as SwingCandle[]);
    const pattern1H = swings1H.slice(-3).map(s => s.swing).join('-');
    if (pattern1H !== sameDirection) return null;
