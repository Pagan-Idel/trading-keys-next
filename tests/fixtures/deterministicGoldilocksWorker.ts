...
const warmup=Array.from({length:14},(_,index)=>({
  time:new Date((base-(14-index)*300)*1000).toISOString(),candleIndex:index,
  open:101.2,high:101.7,low:100.7,close:index%2===0?101.4:101.0,
}));
const candles=[
  ...warmup,
  {time:new Date(base*1000).toISOString(),candleIndex:14,open:101,high:102.7,low:100.8,close:102.5},
  {time:new Date((base+300)*1000).toISOString(),candleIndex:15,open:101,high:101.5,low:99.8,close:100.5},
  {time:new Date((base+600)*1000).toISOString(),candleIndex:16,open:100.8,high:103.2,low:100.4,close:102.2},
];
const written=upsertArchivedCandles({pair,timeframe:'M5',mode:'demo'},candles);
const archived=readArchivedCandles({pair,timeframe:'M5',mode:'demo'},base-4_200,base+901);
...
const confirmations=findFreshGoldilocksConfirmations(history,strategyCandles,300,600_000,strategyCandles,300);
