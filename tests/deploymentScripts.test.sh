...
BIN="$SANDBOX/bin"
mkdir -p "$SEED/pi" "$BIN"
cp "$REPO_UNDER_TEST/pi/deploy.sh" "$REPO_UNDER_TEST/pi/verify-deployment.sh" "$SEED/pi/"
printf '{"scripts":{}}\n' > "$SEED/package.json"
...
cat > "$BIN/curl" <<'EOF'
#!/usr/bin/env sh
test "${MOCK_VERIFY_FAIL:-false}" != true
EOF
cat > "$BIN/pgrep" <<'EOF'
#!/usr/bin/env sh
case "$*" in
...
export TRADING_KEYS_DEPLOY_VALIDATION_COMMAND='mkdir -p artifacts/pi-runtime && touch artifacts/pi-runtime/controlServer.mjs'
