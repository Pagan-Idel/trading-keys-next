...
const record=(overrides:Record<string,unknown>={})=>({config:{
  label:'Leader control',minimumScore:12,datasetKey:'data-a',datasetStartTime:1,datasetEndTime:100,
...
test('research comparison separates dataset changes from strategy settings',()=>{
  const comparison=compareResearchRecords(record(),record({datasetKey:'data-b',datasetStartTime:2,datasetEndTime:175,researchManifest:{capturedAt:'second',versions:{strategy:'v4',codeRevision:'abc',sourceState:'clean'}}}));
