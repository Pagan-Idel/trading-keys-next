...
import { TradeSignal } from "../utils/determineStrategySignal.js";
...
const runStrategyThread = async () => {
  ...
  while (true) {
    try {
      ...
      const dailySwings = determineSwingPoints(daily as SwingCandle[]);
      ...
      while (!passed4H) {
        ...
        const swings4H = determineSwingPoints(fourH as SwingCandle[]);
      ...
      while (!passed1H) {
        ...
        const swings1H = determineSwingPoints(oneH as SwingCandle[]);
      ...
      await logMessage (`❌ Error in strategy thread for ${pair}: ${(err as Error).message}`);
