  ...
  return {
    ...
    zoneRules: [
      ...
      },
      {
        ...
        rule: `${profile.confirmation} owns prior-touch purity through the shared departure qualifier. The first directional completed candle must close beyond the proximal boundary with body/range >= 0.50, body/prior ATR14 >= 0.25, and close penetration/prior ATR14 >= 0.25. ATR uses only the 14 completed candles before the candidate. Every later completed ${profile.confirmation} candle intersecting the zone counts individually, including consecutive candles; the qualifying departure candle and trade-trigger candle are excluded. The departure candle's wick may still overlap the zone.`,
      },
      {
    ...
    confirmationAndExecution: [
      ...
      },
      {
        name: "Break-even",
        rule:
          "At +1R, move the stop to entry, bank 50%, remove the take-profit, and trail the remaining 50% with 2x ATR(14).",
        value: { activationR: 1 },
      },
      {
    ...
    researchDiagnostics: [
      {
        ...
        scored: true,
        rule: `Causal pre-touch evidence: two scored warnings for a confirmed liquidity sweep and a fast momentum approach. Every completed ${profile.confirmation} candle from the first qualifying departure through the candle before first touch defines the available causal span. Both diagnostics use only its return toward the base: from the latest lowest low for supply, or latest highest high for demand, through the candle before first touch. Long return legs retain that exact interval but aggregate to the smallest standard higher resolution producing at most 500 candles. There is no fixed lookback count. A standard sweep clears and reclaims either a four-candle sideways pool or two separated equal-edge pivots, then recovers at least 1.25 ATR through the pool midpoint. The alternate deep-sweep path does not require equal lows or highs: it must clear one prior swing by at least 3.25 ATR and reclaim it with at least 1.25 ATR of adverse recovery within three analysis candles. A newer adverse extreme cancels either pattern. Compression is retained as favorable/neutral context and never deducts points. Fast approach segments distinct multi-candle pushes whenever price pulls back at least 0.35 prior ATR. A push qualifies when its net close displacement toward the zone reaches at least 2 prior ATR and 1.25 zone widths with at least 60% close-path efficiency and at least two advancing close-to-close steps. Each qualifying push is marked at its source-timeframe origin: the highest high before an attack into demand or the lowest low before an attack into supply. One or more pushes create the single fast-warning category. Every confirmed sweep and its recovery form one warning category, not separate deductions. The binary close-through confirmation remains the entry trigger; confirmation-strength research fields do not affect this warning score.`,
        thresholds: {
          minimumPoolCandles: 4,
          minimumStructuralPoolPivots: 2,
          maximumPoolEdgeSpreadAtr: 0.25,
          maximumPoolRangeAtr: 1.5,
          maximumPoolCloseDriftAtr: 0.75,
          minimumPoolAverageOverlapFraction: 0.35,
          minimumSweepExcursionAtr: 0.15,
          minimumSweepExcursionZoneWidth: 0.01,
          minimumSweepReclaimAtr: 0.02,
          recoveryDisplacementAtr: 1.25,
          deepSweepMinimumExcursionAtr: 3.25,
          deepSweepMaximumRecoveryCandles: 3,
          structuralPoolReactionAtr: 1,
